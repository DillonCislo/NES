...
public:
                           ...
                           const LBFGSParam<Scalar>& param)
    {
        ...
        if(param.linesearch != LBFGS_LINESEARCH_BACKTRACKING_STRONG_WOLFE) {
            std::cout << "'param.linesearch' must be 'LBFGS_LINESEARCH_BACKTRACKING_STRONG_WOLFE' for LineSearchNocedalWright" << std::endl;
        ...
        if(dg_init > 0) {
            std::cout << "the moving direction increases the objective function value" << std::endl;
        ...
        for(;;)
        {
          ...
          {
            if( step == step_hi ) {
              std::cout << "the line search method failed, possibly due to insufficient numeric precision" << std::endl;
